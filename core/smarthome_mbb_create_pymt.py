...
def setup_logging(log_file='SMARTHOME_MBB_finance_workflow.log'):
    current_date = datetime.now().strftime("%d%m%Y_%H%M")
    log_file = os.path.join('logs', f'{current_date}_SMARTHOME_MBB_finance_workflow.log')
...
class FinanceWorkflow:
    def __init__(self, csv_file='data/temp/Smarthome_MBB_2025_processed.csv'):
        ...
        self.company_id = os.getenv('SMARTHOME_COMPANY_ID')
        self.journal_id = os.getenv('SMARTHOME_MBB_JOURNAL_ID')

        required_env_vars = ['TENANT_ID', 'CLIENT_ID', 'CLIENT_SECRET', 'BASE_URL', 'SMARTHOME_COMPANY_ID', 'SMARTHOME_MBB_JOURNAL_ID']
...
def main():
    workflow = FinanceWorkflow('data/temp/Smarthome_MBB_2025_processed.csv')
    workflow.process()
